	...
	$.fn.zyUpload = function(options,param){
		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
					// 创建带有拖动的html
					html += '<form id="uploadForm" action="upload_file.php" method="post" enctype="multipart/form-data">';
					html += '	<div class="upload_box">';
					html += '		<div class="upload_main">';
					html += '			<div class="upload_choose">';
	            	html += '				<div class="convent_choice">';
	            	html += '					<div class="andArea">';
	            	html += '						<div class="filePicker">选择上传文件</div>';
	            	html += '						<input id="fileImage" type="file" name="file" id="file" required />';
	            	html += '					</div>';
	            	html += '				</div>';
					//html += '				<span id="fileDragArea" class="upload_drag_area">或者将文件拖到此处</span>';
					html += '				<span class="upload_drag_area"></span>';
					html += '			</div>';
		            html += '			<div class="status_bar">';
		            html += '				<div id="status_info" class="info">选中0张文件，共0B。</div>';
		            html += '				<div class="btns">';
		            //html += '					<div class="webuploader_pick"></div>';
		            html += '					<input class="upload_btn2" type="submit" name="submit" value="开始上传" />';
		            html += '				</div>';
		            html += '			</div>';
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				if (file.type.indexOf("image") == 0) {
					...
					html += '		</div>';
					html += '	</div>';
					html += '	<a style="height:'+para.itemHeight+';width:'+para.itemWidth+';" href="http://10.187.108.185:8000/upload_file.php" class="imgBox">';
					html += '		<div class="uploadImg" style="width:'+imgWidth+'px">';
			...
			this.createCorePlug = function(){
				var params = {
					...
					onSuccess: function(file, response) {
						...
						$("#uploadInf").append("<p>上传成功!" + "<br/>" + response + "</p>");
						// 根据配置参数确定隐不隐藏上传成功的文件
						if(para.finishDel){
							// 移除效果
							$("#uploadList_" + file.index).fadeOut();
